        ...
        try:
            with tempfile.NamedTemporaryFile(suffix=".zip", delete=False) as temp_zip:
                ...
                try:
                    ...
                    thumb = processor.generate_thumbnail(images[0]['content'])
                    if thumb:
                        chapter.thumbnail.save(f"{chapter.title}_thumbnail.jpg", ContentFile(thumb), save=True)
            ...
            if temp_zip_path and os.path.exists(temp_zip_path):
                try:
                    ...
                    logger.warning(f"Could not delete temp file {temp_zip_path}: {e}")
